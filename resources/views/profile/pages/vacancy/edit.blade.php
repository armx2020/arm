        ...
        <div class="flex flex-col basis-full lg:basis-4/5 lg:m-3 my-3 lg:ml-5">
            <div class="flex flex-col basis-full">
                <div class="flex flex-col md:flex-row basis-full bg-white rounded-md p-1 lg:p-10 relative">
                    <form method="post" action="{{ route('myvacancies.update', ['myvacancy' => $vacancy->id]) }}"
                            ...
                            <div class="flex relative">
                                ...
                                @else
                                    <img class="h-10 w-10 rounded-lg m-4 object-cover" id="img"
                                        src="{{ asset('storage/' . $vacancy->image) }}" alt="image">
                                @endif
                                <button type="button" id="remove_image" class="absolute top-5 right-5"
                                    @if ($vacancy->image == null) style="display: none;"
                            @else
                        ...
                        <div class="my-3">
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                                :value="old('name', $vacancy->name)" required autofocus />
                        ...
                        <div class="my-3">
                            ...
                            <x-text-input id="address" name="address" type="text" class="mt-1 block w-full"
                                :value="old('address', $vacancy->address)" autofocus />
                        ...
                        <div class="my-3">
                            ...
                            <x-text-input id="description" name="description" type="text" class="mt-1 block w-full"
                                :value="old('description', $vacancy->description)" autofocus />
                        ...
                        <div class="my-3">
                            ...
                            <select name="parent" id="parent"
                                ...
                                required>
                                <option
                                    @if ($vacancy->parent_type == 'App\Models\User') value="User|{{ $vacancy->parent->id }}"
                            @elseif ($vacancy->parent_type == 'App\Models\Company')
                            value="Company|{{ $vacancy->parent->id }}"
                            @else ($vacancy->parent_type == 'App\Models\Group')
                            value="Group|{{ $vacancy->parent->id }}" @endif>
                                    {{ $vacancy->parent->name ? $vacancy->parent->name : $vacancy->parent->firstname }}
                                    {{ $vacancy->parent->lastname }}</option>
                                <option disabled>-выберите инициатора-</option>
                                <option value="User|{{ Auth::user()->id }}">{{ Auth::user()->firstname }}
                        ...
                        <div class="my-3">
                            ...
                            <select name="vacancy_city" class="w-full" style="border-color: rgb(209 213 219)"
                                id="vacancy_city">
                ...
                <div class="flex basis-full bg-gray-200 rounded-md p-3 my-6">
                    <form method="post" action="{{ route('myvacancies.destroy', ['myvacancy' => $vacancy->id]) }}"
