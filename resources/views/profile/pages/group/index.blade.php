...
<div class="flex flex-col lg:flex-row w-11/12 mx-auto my-10">
    ...
    <div class="flex flex-col basis-full lg:basis-4/5 lg:m-3 my-3 lg:ml-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 lg:gap-5 w-full">
            <button id="add_group" class="h-80 block rounded-lg border-dashed border-2 border-indigo-600 hover:bg-white">
                <div class="flex flex-col">
                    <div class="text-9xl text-indigo-600 flex mx-auto leading-none">+</div>
                    <div class="text-xl text-indigo-600 flex mx-auto uppercase">Добавить группу</div>
                </div>
            </button>
            @foreach($groups as $group)
            <div class="block rounded-lg bg-white h-80">
                <a href="{{ route('mygroup.show', ['id' => $group->id ]) }}" class="block h-52">
                    @if( $group->image == null )
    ...
    <div id="add_group_form" class="hidden fixed inset-0 px-4 min-h-screen sm:px-0 z-50" focusable>
        ...
        <div class="my-16 mx-auto opacity-100 translate-y-0 sm:scale-100 bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:w-full md:max-w-md lg:max-w-lg ">
            <form method="post" action="{{ route('mygroup.store') }}" class="p-6" enctype="multipart/form-data">
                @csrf
                ...
                <div class="my-2">
                    ...
                    <select name="group_city" class="w-full" style="border-color: rgb(209 213 219); width:100%" id="group_city">
