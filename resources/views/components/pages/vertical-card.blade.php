<div class="flex flex-col rounded-lg bg-white h-80">
    <a href="{{ route($entityShowRout, ['id' => $entity->id]) }}" class="flex">
        @if ($entity->image == null)
            <img class="h-48 w-full rounded-2xl p-2 flex object-cover" src="{{ url('/image/no-image.png') }}"
                alt="image" />
        @else
            <img class="h-48 w-full rounded-2xl p-2 flex object-cover" src="{{ asset('storage/' . $entity->image) }}"
                alt="image">
        @endif
    </a>
    <div class="px-3 md:px-6">
        ...
        <div class="relative">
            <div class="text-left pb-0 block">
                @if ($entity->viber)
                    <a href="{{ $entity->viber }}" class="inline">
                ...
                @if ($entity->web)
                    <a href="{{ $entity->web }}" class="inline">
                        <svg class="w-5 h-5 inline" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1"
