...
<nav class="w-11/12 mb-2 mt-5 rounded-md mx-auto px-3 lg:px-2 text-sm md:text-md">
    <ol class="list-reset flex">
        ...
        <li>
            <a href="" class="text-primary transition duration-150 ease-in-out hover:text-primary-600 focus:text-primary-600 active:text-primary-700 dark:text-primary-400 dark:hover:text-primary-500 dark:focus:text-primary-500 dark:active:text-primary-600">Пользователи</a>
        </li>
        <li>
            <span class="mx-2 text-neutral-500 dark:text-neutral-400">/</span>
        </li>
    ...
    <div class="flex flex-col w-11/12 mx-auto my-6 lg:my-8">
        <div class="flex flex-col md:flex-row basis-full bg-white rounded-md p-2 lg:p-10 relative">
            ...
            <div class="flex flex-col px-3 lg:px-10">
                <h3 class="text-left text-xl lg:text-2xl m-2 md:m-5">{{ $user->firstname}} {{ $user->lastname}}</h1>
                    <p class="text-left text-md mx-4 my-1 text-gray-600">{{ $user->city->name }} ({{$user->region->name }})</p>
                    ...
                    <div class="absolute right-1 top-1 lg:right-3 lg:top-3">
                        <div class="m-3 break-all text-base text-right ">
                            <p class="text-left text-md mx-4 my-1 text-blue-500 drop-shadow-md">
                                        @if(now()->diffInSeconds($user->last_active_at) < 300)
                                            онлайн 
                                        @elseif (now()->diffInSeconds($user->last_active_at) > 300 && now()->diffInSeconds($user->last_active_at) <= 3600)
                                            был(а) недавно 
                                        @elseif (now()->diffInSeconds($user->last_active_at) > 3600 && now()->diffInSeconds($user->last_active_at) <= 86400)
                                            был(а) сегодня
                                        @elseif (now()->diffInSeconds($user->last_active_at) > 86400 && now()->diffInSeconds($user->last_active_at) <= 604800)
                                            был(а) в течение недели 
                                        @else 
                                            был(а) давно
                                        @endif
                            </p>
