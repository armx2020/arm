...
<nav class="w-11/12 mb-2 mt-5 rounded-md mx-auto px-3 lg:px-2 text-sm md:text-md">
    <ol class="list-reset flex">
        ...
        <li>
            <a href="{{ route('vacancy.index') }}" class="text-primary transition duration-150 ease-in-out hover:text-primary-600 focus:text-primary-600 active:text-primary-700 dark:text-primary-400 dark:hover:text-primary-500 dark:focus:text-primary-500 dark:active:text-primary-600">Резюме</a>
        </li>
        <li>
            <span class="mx-2 text-neutral-500 dark:text-neutral-400">/</span>
        </li>
    ...
    <div class="flex flex-col w-11/12 mx-auto my-6 lg:my-8">
        <div class="flex flex-col md:flex-row basis-full bg-white rounded-md p-1 lg:p-10 relative">
            <div class="flex flex-col basis-1/5">
                ...
                @else
                <img class="h-56 w-full rounded-2xl p-2 flex object-cover" src="{{ asset( 'storage/'.$resume->user->image) }}" alt="image">
                @endif
            ...
            <div class="flex flex-col px-3 lg:px-10 basis-3/4">
                <h3 class="text-left text-xl lg:text-2xl mx-4">{{ $resume->name }} ({{$resume->user->firstname}} {{ $resume->user->lastname}})</h1>
                    <p class="text-left text-md mx-4 my-1 text-gray-600">{{ $resume->city->name }} ({{$resume->region->name }})
                        <br>{{ $resume->address }}
                    </p>
                    <p class="text-left text-sm mx-4 my-1 text-gray-500 break-all">{{ $resume->description }}</p>
                    <hr class="mt-3 mb-3">
                    <div class="flow-root mb-3 break-words">
                        ...
                        <div class="inline">
                            ...
                            {{ $resume->user->whatsapp }}
                        </div>
                        @endif

                        @if( $resume->user->instagram )
                        <div class="inline">
                            ...
                            {{ $resume->user->instagram }}
                        </div>
                        @endif

                        @if( $resume->user->vkontakte )
                        <div class="inline">
                            ...
                            {{ $resume->user->vkontakte }}
                        </div>
                        @endif

                        @if( $resume->user->telegram )
                        <div class="inline">
