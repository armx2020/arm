...
<nav class="w-11/12 mb-2 mt-5 rounded-md mx-auto px-3 lg:px-2 text-sm md:text-md">
    <ol class="list-reset flex">
        ...
        <li>
            <a href="{{ route('vacancy.index') }}" class="text-primary transition duration-150 ease-in-out hover:text-primary-600 focus:text-primary-600 active:text-primary-700 dark:text-primary-400 dark:hover:text-primary-500 dark:focus:text-primary-500 dark:active:text-primary-600">Вакансии</a>
        </li>
        <li>
            <span class="mx-2 text-neutral-500 dark:text-neutral-400">/</span>
        </li>
    ...
    <div class="flex flex-col w-11/12 mx-auto my-6 lg:my-8">
        <div class="flex flex-col md:flex-row basis-full bg-white rounded-md p-1 lg:p-10 relative">
            <div class="flex flex-col basis-1/5">
                ...
                @else
                <img class="h-56 w-full rounded-2xl p-2 flex object-cover" src="{{ asset( 'storage/'.$vacancy->parent->image) }}" alt="image">
                @endif
            ...
            <div class="flex flex-col px-3 lg:px-10 basis-3/4">
                <h3 class="text-left text-xl lg:text-2xl mx-4">{{ $vacancy->name }}</h1>
                    <p class="text-left text-md mx-4 my-1 text-gray-600">{{ $vacancy->city->name }} ({{$vacancy->region->name }})
                        <br>{{ $vacancy->address }}
                    </p>
                    <p class="text-left text-sm mx-4 my-1 text-gray-500 break-all">{{ $vacancy->description }}</p>
                    <hr class="mt-3 mb-3">
                    <div class="flow-root mb-3 break-words">

                        @if($vacancy->parent->phone)
                        <div class="inline">
                            ...
                            {{ $vacancy->parent->phone }}
                        </div>
                        @endif

                        @if($vacancy->parent->viber)
                        <div class="inline">
                            ...
                            {{ $vacancy->parent->viber }}
                        </div>
                        @endif

                        @if( $vacancy->parent->web )
                        <div class="inline">
                        ...
                        <div class="inline">
                            ...
                            {{ $vacancy->parent->whatsapp }}
                        </div>
                        @endif

                        @if( $vacancy->parent->instagram )
                        <div class="inline">
                            ...
                            {{ $vacancy->parent->instagram }}
                        </div>
                        @endif

                        @if( $vacancy->parent->vkontakte )
                        <div class="inline">
                            ...
                            {{ $vacancy->parent->vkontakte }}
                        </div>
                        @endif

                        @if( $vacancy->parent->telegram )
                        <div class="inline">
