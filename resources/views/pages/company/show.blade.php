    ...
    <section>
        <div class="flex flex-col mx-auto my-6 lg:my-8">
            <div class="flex flex-col md:flex-row basis-full bg-white rounded-md p-1 lg:p-10">
                <div class="flex flex-col basis-1/5" @if ($company->image !== null && $company->image1 !== null) id="slider" @endif>
                    <ul>
                        @if ($company->image == null)
                            <img src="{{ url('/image/no-image.png') }}" class="h-56 w-full rounded-2xl p-2 flex object-cover"
                                alt="image" />
                        @else
                        ...
                        @if ($company->image1)
                            <li><img src="{{ asset('storage/' . $company->image1) }}"
                                    class="h-56 w-full rounded-2xl p-2 flex object-cover" alt="image1"></li>
                        @endif

                        @if ($company->image2)
                            <li><img src="{{ asset('storage/' . $company->image2) }}"
                                    class="h-56 w-full rounded-2xl p-2 flex object-cover" alt="image2"></li>
                        @endif

                        @if ($company->image3)
                            <li><img src="{{ asset('storage/' . $company->image3) }}"
                                    class="h-56 w-full rounded-2xl p-2 flex object-cover" alt="image3"></li>
                        @endif

                        @if ($company->image4)
                            <li><img src="{{ asset('storage/' . $company->image4) }}"
                                    class="h-56 w-full rounded-2xl p-2 flex object-cover" alt="image4"></li>
                        @endif
                    </ul>
                </div>
                <div class="flex flex-col px-3 lg:px-10">
                    <h3 class="text-left text-xl lg:text-2xl mx-4">{{ $company->name }}</h3>
                    <p class="text-left text-md mx-4 my-1 text-gray-600">{{ $company->city->name }}
                        {{ $company->address }}
                    </p>
                        ...
                        @if ($company->viber)
                            <div class="inline m-2">
                                <svg class="w-5 h-5 inline my-2" xmlns="http://www.w3.org/2000/svg"
                                    xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 192 210.0428"
                                    id="viber" version="1.1" viewBox="0 0 192 210.0428" xml:space="preserve">
                                    <g>
                                        <path
