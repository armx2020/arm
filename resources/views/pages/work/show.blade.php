...
<nav class="mb-2 mt-5 rounded-md mx-auto px-3 lg:px-2 text-sm md:text-md">
    <ol class="list-reset flex">
        ...
        <li>
            <a href="{{ route('works.index') }}" class="text-primary transition duration-150 ease-in-out hover:text-primary-600 focus:text-primary-600 active:text-primary-700 dark:text-primary-400 dark:hover:text-primary-500 dark:focus:text-primary-500 dark:active:text-primary-600">
                @if($work->type == 'vacancy')
                Вакансии
                @else
                Резюмэ
                @endif
    ...
    <div class="flex flex-col mx-auto my-6 lg:my-8">
        <div class="flex flex-col md:flex-row basis-full bg-white rounded-md p-1 lg:p-10 relative">
            ...
            <div class="flex flex-col px-3 lg:px-10 basis-3/4">
                <h3 class="text-left text-xl lg:text-2xl mx-4">{{ $work->name }}</h3>
                    <p class="text-left text-md mx-4 my-1 text-gray-600">{{ $work->city->name }} ({{$work->region->name }})
                        <br>{{ $work->address }}
                    </p>
                    <p class="text-left text-sm mx-4 my-1 text-gray-500 break-all">{{ $work->description }}</p>
                    <hr class="mt-3 mb-3">
                    <div class="flow-root mb-3 break-words">

                        @if($work->parent->phone)
                        ...
                        <div class="inline m-2">
                            ...
                            {{ $work->parent->whatsapp }}
                        </div>
                        @endif

                        @if( $work->parent->instagram )
                        <div class="inline m-2">
                            ...
                            {{ $work->parent->instagram }}
                        </div>
                        @endif

                        @if( $work->parent->vkontakte )
                        <div class="inline m-2">
                            ...
                            {{ $work->parent->vkontakte }}
                        </div>
                        @endif

                        @if( $work->parent->telegram )
                        <div class="inline m-2">
