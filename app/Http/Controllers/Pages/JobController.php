...
class JobController extends BaseController
{
    ...
    public function index(Request $request, $category = null, $subCategory = null)
    {
        ...
        $entities = Entity::query()->active()->jobs()->with('fields', 'offers')->withCount('offers');
        $categories = Category::query()->main()->active()->jobs()->get();
        $subCategories = null;

        if ($category) {
            $category = Category::active()->main()->jobs()->select('id', 'transcription')->where('transcription', $category)->First();
            ...
            if ($subCategory) {
                $subCategory = Category::active()->jobs()->select('id', 'transcription')->where('transcription', $subCategory)->First();
